    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-md flex flex-col h-full bg-card">
        <SheetHeader className="flex-shrink-0">
          ...
          <SheetDescription>
            {totalItems === 0 ? "Sua sacola está vazia" : `${totalItems} ${totalItems !== 1 ? 'itens' : 'item'}`}
          </SheetDescription>
        </SheetHeader>
          ...
          {items.length === 0 ? (
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0 space-y-4">
                {items.map((item) => (
                  <div key={item.variantId} className="flex gap-3 p-2 border-b border-border">
              ...
              <div className="flex-shrink-0 space-y-3 pt-4 border-t border-border">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-lg">R$ {totalPrice.toFixed(2)}</span>
                </div>
